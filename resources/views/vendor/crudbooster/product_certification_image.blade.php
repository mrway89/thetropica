...
@section('content')
            ...
            $(function () {
                ...
                var sortactive = $(".draggable-menu").sortable({
                    ...
                    onDrop: function ($item, container, _super) {
                        ...
                        $.post("{{route('AdminProductCertificationControllerPostSaveSorting')}}", {images: jsonString, item_id: item_id}, function (resp) {
                ...
                $('.set_default').on('click', function() {
                    var id          = $(this).data('id');
                    var defaultUrl  = '{{route('AdminProductCertificationControllerPostSetDefault')}}';
    ...
    <div class='row'>
        ...
        <div class="col-sm-5">
            <div style="margin-bottom: 10px;"> <a href="{{'products'}}" class="btn btn-sm btn-danger"><i class="fa fa-chevron-left"></i> Back to Product Management</a></div>
                        ...
                        @foreach($images as $image)
                            <li data-id='{{$image->id}}'>
                                <div class='' title="" style='text-align: center;'>
                                    <img src="{{ asset($image->url) }}" width="80">
                                    <span class='pull-right'>
                                        {{-- <a title='Default' data-id="{{ $image->id }}"
                                        class="set_default {{ $image->is_featured ? 'text-success' : 'text-info'}}"
                                        href='javascript:void(0)' style="margin-right:5px;">{{ $image->is_featured ? 'Default' : 'Set as Default'}}</a> --}}
                                        <a class='fa fa-pencil text-white' title='Edit' href='{{route("AdminProductCertificationControllerGetEdit",["id"=>$image->id])}}?return_url={{urlencode(Request::fullUrl())}}'></a>
                                        &nbsp;&nbsp;
                                        <a title='Delete' class='fa fa-trash' onclick='{{CRUDBooster::deleteConfirm(route("AdminProductCertificationControllerGetDelete",["id"=>$image->id]))}}' href='javascript:void(0)'></a>
                                    </span>
                                </div>
                            </li>
                        @endforeach
