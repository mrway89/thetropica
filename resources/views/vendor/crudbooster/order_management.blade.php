...
<div class="row clearfix">
    <div class="col-lg-12" id="printableArea">
      <div class='panel panel-default'>
            <div class="panel-heading print_hide">
                <h4 class="print_hide">Order Detail</h4>
                <h4 class="print_hide hide_normal">Invoice</h4>
            </div>
            <div class="panel-body">
                <div class="row print_hide">
                    <div class="col-sm-12">
                        <a href="{{route('AdminOrdersControllerGetCheckOrder', $order->id)}}" class="btn btn-raised btn-info pull-right" style="margin-right:5px;">
                            <i class="fa fa-refresh margin-r-5"></i>&nbsp;Check Payment
                        </a>
                        ...
                        <a href="{{route('AdminOrdersControllerGetGenerateDelivery', $order->id)}}" class="btn btn-raised btn-success pull-right" style="margin-right: 5px;">
                            <i class="fa fa-download margin-r-5"></i> Generate Delivery Label
                        </a>
                        <a href="javascript:void(0);"  onclick="printDiv('printableArea')" class="btn btn-raised btn-success pull-right" style="margin-right:5px;">
                            <i class="fa fa-print"></i>
                        </a>
                        {{-- <a href="{{ route('AdminOrdersControllerGetPrintAddress', $order->order_code) }}" class="btn btn-raised btn-success pull-right" style="margin-right:5px;">
                            <i class="fa fa-print"></i> Print Delivery
                        </a> --}}
                        {{-- <a href="javascript:void(0);"  onclick="printDivNoPrice('printableArea')" class="btn btn-raised btn-success pull-right" style="margin-right:5px;">
                            <i class="fa fa-print"></i> No Price
                        </a> --}}
                    </div>
                </div>
                <div class="row clearfix" style="margin-top:20px;">
                    <div class="col-md-12">
                        ...
                        <h4 class="float-md-right code_print">
                ...
                <div class="row mt-3" style="margin-top:20px;">
                    <div class="col-md-4 col-sm-12">
                        ...
                        <address>
                            ...
                            {{ $order->cart->address->address }}, {{ $order->cart->address->city }}<br />
                    ...
                    <div class="col-md-4 col-xs-12">
                        ...
                        <button class="btn btn-raised btn-info btn-action print_hide" data-status="1"><i class="fa fa-check margin-r-5"></i> Set as Paid</button>
                        @endif
                        @if($order->status == 'paid')
                        <button class="btn btn-raised btn-success btn-action print_hide" data-status="2"><i class="fa fa-check margin-r-5"></i> Set on Delivery</button>
                        @endif
                        @if($order->status == 'sent')
                        <button class="btn btn-raised btn-success btn-action print_hide" data-status="3"><i class="fa fa-check margin-r-5"></i> Set as Completed</button>
                        @endif
                        {{-- <button class="btn btn-raised btn-danger btn-action" data-status="4"><i class="fa fa-times margin-r-5"></i> Cancel</button> --}}
                    </div>
                    <div class="col-md-4 col-sm-12 col-xs-12 text-right print_right">
                        <p>
                            <strong>Order Date: </strong>
                            {{ $order->created_at->format('d M Y, H:i:s') }}
                        </p>
                        <p class="print_hide" style="margin-top:10px;">
                            ...
                            <table id="mainTable" class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle print_hide">
                                        ...
                                        @endphp
                                        <td class="align-middle">{{ $detail->product->full_name }} {{ $confirm_label }}</td>
                                        <td class="align-middle">{{ $detail->quantity }}</td>
                                        <td class="align-middle hide_price">{{ currency_format($detail->price) }}</td>
                                        <td class="align-middle hide_price">{{ currency_format($detail->price * $detail->quantity) }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 text-right hide_price">
                        <p class="text-right">
                            <b>Sub-total:</b>
                        ...
                        @endif
                        <hr>
                        <h3 class="text-right total_print">{{ currency_format($order->grand_total) }}</h3>
                    </div>
                </div>
                <hr>
...
@media print
{
    * { font-size:12px; }
    ...
    .print_right { float: right; text-align: left !important; }
    .total_print { font-size:14px; }
    .code_print strong {
        position: absolute;
        top: 0;
        right: 0;
...
function printDivNoPrice(divName) {
     var printContents = document.getElementById(divName).innerHTML;
     var originalContents = document.body.innerHTML;

     $('.hide_price').addClass('hidden');
