            ...
            $(function () {
                ...
                var sortactive = $(".draggable-menu").sortable({
                    ...
                    onDrop: function ($item, container, _super) {
                        ...
                        $.post("{{route('AdminHomeSlideshowControllerPostSaveSorting')}}", {images: jsonString, item_id: item_id}, function (resp) {
    ...
    <div class='row'>
            ...
            <div class="panel panel-success">
                <div class="panel-heading">
                    <strong>Slideshow Order</strong> <span id='menu-saved-info' style="display:none" class='pull-right text-success'><i class='fa fa-check'></i> Image Saved !</span>
                </div>
                            ...
                            <li data-id='{{$image->id}}'>
                                <div class='position-relative' title="" style="text-align: center;">
                                    ...
                                    <span class='edit-icon'>
                                        <a class='fa fa-pencil text-white' title='Edit' href='{{route("AdminHomeSlideshowControllerGetEdit",["id"=>$image->id])}}?return_url={{urlencode(Request::fullUrl())}}' style="; background: #00a65a;"></a>

                                        <a title='Delete' class='fa fa-trash text-white' onclick='{{CRUDBooster::deleteConfirm(route("AdminHomeSlideshowControllerGetDelete",["id"=>$image->id]))}}' href='javascript:void(0)'></a>
                                    </span>
                                    <span class="banner-content">
                                        <b>{{ $image->title_id ? $image->title_id : 'Empty title' }}</b><br>
                                        {{ $image->content_id ? $image->content_id : 'Empty content' }}
                                    </span>
                                    {{-- <span class="banner-link">
                                        {{ $image->link ? $image->link : 'Empty link' }}
                                    </span> --}}
                                </div>
                            </li>
                        @endforeach
