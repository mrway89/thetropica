...
<div class="section fp-table about-area aboutbg minh-100vh" id="about-{{ $i }}" style="background: url({{ asset($post->url) }}?w=1920);">
	<div class="fp-tableCell px-3 minh-100vh d-flex justify-content-center align-items-center">
		<div class="row d-flex justify-content-center align-items-center pt-md-0 pt-5">
	        <div class="col-md-12 text-center text-white ">
	            <h3 class="mb-0  ">{{ $post->{'subtitle_' . $language} }}</h3>
	            <h1 class="mb-md-5 mb-1">{{ ucwords($post->{'title_' . $language}) }}</h1>
	        </div>
...
<div class="section fp-table contact-about-area" id="about-{{ $posts->count() + 1 }}" style="background: url({{ asset($bgContact->url) }}?w=1920);">
		...
		<div class="container container-product">
			<div class="row justify-content-center pt-5">
		        <div class="col-md-5 text-white pt-mb-0 pt-5">
					...
					<div class="w-100 float-left hidden-768">
	...
	<div class="discover discover-product">
        ...
        {{-- <a href="#{{ $posts->count() + 2 }}" title="">
        	<img src="{{ asset('assets/img/down-button.png') }}" alt="">
        </a> --}}
    </div>
</div>*/ ?>

{{-- <div class="section fp-table contact-about-area" id="about-{{ $posts->count() + 2 }}" style="background: url({{ asset($bgPost->url) }}?w=1920);">
	<div id="slidepage" class="carousel slide" data-ride="carousel">
		<div class="carousel-inner our-carousel mt-md-0 mt-5">
			@foreach ($news as $i=>$post)
				<div class="carousel-item {{($i==0 ? 'active':'')}}">
					<div class="container container-product">
						<div class="row text-white justify-content-center">
							<div class="col-md-5 text-white">
									<div class="address mb-4 mt-md-5 mt-5 pt-md-5 pt-3">
											<p>{{ ucwords($post->category->{'title_' . $language}) }}</p>
									</div>
									<h1 class="mb-4 w-75">{{ $post->{'title_' . $language} }}</h1>
									<div class="w-100 float-left our-social-img-mb visible-768">
										<img src="{{asset('assets/img/andreas-wagner-294322-unsplash.jpg')}}"/>
									</div>
									<div class="address w-100 float-left ">
										@php
												if ($language == 'id') {
												...
												$jsonPost = json_decode($post->cover);
										@endphp
										@if ($jsonPost->start_date)
											<h4 class="mb-3 date-our">{{ Carbon\Carbon::parse($jsonPost->start_date)->formatLocalized('%B') }} {{ Carbon\Carbon::parse($jsonPost->start_date)->format('j') }}-{{ Carbon\Carbon::parse($jsonPost->end_date)->format('j') }}, {{ Carbon\Carbon::parse($jsonPost->start_date)->format('Y') }}</h4>
										@endif

											<div class="desc-our mb-1">
												<p class="line-height-15">
													{!! $post->{'content_' . $language} !!}
												</p>
											</div>

											<div class="line-height-15 cl-white readmore" onclick="readmore()"><a href="#">Read More</a></div>
									</div>

							</div>
							<div class="col-md-7 pt-5 hidden-768">
								<div class="outer">
									<div class="owl-carousel owl-theme big" id="big_carousel_{{ $i }}">
									...
									<div class="owl-carousel owl-theme thumbs" id="thumbs_carousel_{{ $i }}">
										@foreach ($post->images as $image)
										<div class="item">
												<img src="{{ asset($image->url) }}" alt="">
										</div>
				...
				@endforeach
				@if ($news->count() > 1)
					<a class="carousel-control-prev" href="#slidepage" role="button" data-slide="prev">
							<img src="{{ asset('assets/img/arrow-left.png') }}" class="arrow-left" alt="">
					</a>
					<a class="carousel-control-next" href="#slidepage" role="button" data-slide="next">
