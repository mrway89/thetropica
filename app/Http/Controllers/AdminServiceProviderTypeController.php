...
class AdminServiceProviderTypeController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public function cbInit()
    {
        ...
        $this->form[] = ['label' => 'Cover Image', 'name' => 'cover_image', 'type' => 'upload', 'validation' => 'image|max:1000', 'upload_encrypt' => 'true', 'width' => 'col-sm-10', 'help' => 'Best resolution: 600 x 400px, Max file Size: 1 Mb'];
        $this->form[] = ['label' => 'Banner Image', 'name' => 'banner_image', 'type' => 'upload', 'validation' => 'image|max:1000', 'upload_encrypt' => 'true', 'width' => 'col-sm-10', 'help' => 'Best resolution: 860 x 320px, Max file Size: 1 Mb'];
        $this->form[] = ['label' => 'Description', 'name' => 'description', 'type' => 'textarea', 'validation' => ''];
    ...
    */
    ...
    {
        $query->where('type', 'service_provider_type');
    }
    ...
    */
    ...
    {
        $count                  = Category::where('type', 'service_provider_type')->count();
        $slug                   = str_slug($postdata['title']);
        $checkSlug              = Category::where('type', 'service_provider_type')->where('slug', str_slug($postdata['title']))->get()->count();
        if ($checkSlug > 0) {
            $slug = str_slug($postdata['title'] . '-' . str_random(5));
        }
        $postdata['slug']       = $slug;
        $postdata['type']       = 'service_provider_type';
    }
    ...
    */
    public function hook_after_add($id)
    {
        ...
        $logDesc    = 'Add new service provider type ' . $category->title;
        insertRhapsodieLog($logDesc, 'service_provider_type', CRUDBooster::myId(), $category->toJson());
    }
    ...
    */
    public function hook_before_edit(&$postdata, $id)
    {
        ...
        $postdata['slug']       = $slug;
        $category               = Category::findOrFail($id);
        $logDesc                = 'Edit service provider type ' . $category->title;
        insertRhapsodieLog($logDesc, 'service_provider_type', CRUDBooster::myId(), json_encode(array_diff($postdata, $category->toArray())));
    }
    ...
    */
    ...
    {
        // $category     = Category::findOrFail($id);
        // $logDesc      = 'Delete service provider type ' . $category->title;
        // insertRhapsodieLog($logDesc, 'payment_instruction', CRUDBooster::myId(), $category->toJson());
    }
