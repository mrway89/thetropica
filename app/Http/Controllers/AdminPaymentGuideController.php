...
class AdminPaymentGuideController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public function cbInit()
    {
        ...
        $max_sorting        = DB::table($this->table)->where('type', 'payment_guide')->max('sorting');
    ...
    */
    ...
    {
        $query->where('type', 'payment_guide');
    }
    ...
    */
    public function hook_before_add(&$postdata)
    {
        ...
        $count                  = Content::where('type', 'payment_guide')->count();
        $slug                   = str_slug($postdata['title']);
        $checkSlug              = Content::where('type', 'payment_guide')->where('slug', str_slug($postdata['title']))->get()->count();
        if ($checkSlug > 0) {
            $slug = str_slug($postdata['title'] . '-' . str_random(5));
        }
        $postdata['slug']       = $slug;
        $postdata['type']       = 'payment_guide';
    ...
    */
    public function hook_after_add($id)
    {
        ...
        $logDesc      = 'Add new payment guide ' . $how->title;
        insertRhapsodieLog($logDesc, 'payment_guide', CRUDBooster::myId(), $how->toJson());
    }
    ...
    */
    public function hook_before_edit(&$postdata, $id)
    {
        ...
        $logDesc      = 'Edit payment guide ' . $content->title;
        insertRhapsodieLog($logDesc, 'payment_guide', CRUDBooster::myId(), json_encode(array_diff($postdata, $content->toArray())));
    }
    ...
    */
    public function hook_before_delete($id)
    {
        ...
        // $logDesc      = 'Delete payment guide ' . $faq->question;
        // insertRhapsodieLog($logDesc, 'payment_guide', CRUDBooster::myId(), $faq->toJson());
    }
    ...
    */
    ...
    {
        if ($status == 'up') {
            DB::table('contents')->where('type', 'payment_guide')->where('sorting', $sorting - 1)->update(['sorting' => $sorting]);
            DB::table('contents')->where('type', 'payment_guide')->where('id', $id)->update(['sorting' => ($sorting - 1)]);
        } else {
            DB::table('contents')->where('type', 'payment_guide')->where('sorting', $sorting + 1)->update(['sorting' => $sorting]);
            DB::table('contents')->where('type', 'payment_guide')->where('id', $id)->update(['sorting' => ($sorting + 1)]);
        }
