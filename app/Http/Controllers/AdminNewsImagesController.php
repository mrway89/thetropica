...
class AdminNewsImagesController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public function cbInit()
    {
        ...
        $this->form[] = ['label'=>'Image', 'name'=>'url', 'type'=>'upload', 'validation'=>'required|image|max:1000', 'upload_encrypt' => 'true', 'width'=>'col-sm-10', 'help'=>'Best resolution: 1000 x 1000px, Max file Size: 1 Mb'];
        $this->form[] = ['label'=>'type', 'name'=>'type', 'type'=>'hidden', 'validation'=>'', 'width'=>'col-sm-10', 'help'=>'', 'value' => 'news'];
    ...
    */
    ...
    {
        $query->where('type', 'news')->where('item_id', request('parent_id'));
    }
    ...
    */
    ...
    {
        $count                  = Image::where('type', 'news')->where('item_id', $postdata['item_id'])->get()->count();
        $postdata['type']       = 'news';
    ...
    */
    public function hook_after_add($id)
    {
        ...
        $logDesc = 'Add new image to news ' . $image->post->title_id;
        insertRhapsodieLog($logDesc, 'news_images', CRUDBooster::myId(), $image->url);
    }
    ...
    */
    ...
    public function getIndex()
    {
        ...
        $images = DB::table('images')->where('type', 'news')->where('item_id', request('parent_id'))->orderby('sorting', 'asc')->get();

        $submenu =  Post::findOrFail(request('parent_id'));

        $return_url = Request::fullUrl();

        $page_title = 'News Images Management for ' . $submenu->name;

        return view('vendor/crudbooster/news_images', compact('images', 'return_url', 'page_title', 'submenu'));
    }
