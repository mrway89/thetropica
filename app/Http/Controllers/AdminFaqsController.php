...
class AdminFaqsController extends \crocodicstudio\crudbooster\controllers\CBController
{
    ...
    */
    public function hook_before_add(&$postdata)
    {
        ...
        $count                  = Content::where('type', 'faqs')->count();
        $slug                   = str_slug($postdata['title']);
        $checkSlug              = Content::where('type', 'faqs')->where('slug', str_slug($postdata['title']))->get()->count();
        if ($checkSlug > 0) {
    ...
    */
    public function hook_after_add($id)
    {
        ...
        $logDesc      = 'Add new faqs ' . $content->title;
        insertRhapsodieLog($logDesc, 'faqs', CRUDBooster::myId(), $content->toJson());
    }
    ...
    */
    public function hook_before_edit(&$postdata, $id)
    {
        ...
        $content                = Content::findOrFail($id);
        $logDesc                = 'Edit faqs ' . $content->title;
        insertRhapsodieLog($logDesc, 'faqs', CRUDBooster::myId(), json_encode(array_diff($postdata, $content->toArray())));
    }
    ...
    */
    ...
    {
        // if (is_array($id)) {
        //     $faq   = Content::findOrFail($id[0]);
        // } else {
        //     $faq          = Content::findOrFail($id);
        // }
        // $logDesc      = 'Delete news ' . $faq->title_en;
        // insertRhapsodieLog($logDesc, 'faqs', CRUDBooster::myId(), $faq->toJson());
    }
    ...
    */
    ...
    {
        if ($status == 'up') {
            DB::table('contents')->where('type', 'faqs')->where('sorting', $sorting - 1)->update(['sorting' => $sorting]);
            DB::table('contents')->where('type', 'faqs')->where('id', $id)->update(['sorting' => ($sorting - 1)]);
        } else {
            DB::table('contents')->where('type', 'faqs')->where('sorting', $sorting + 1)->update(['sorting' => $sorting]);
            DB::table('contents')->where('type', 'faqs')->where('id', $id)->update(['sorting' => ($sorting + 1)]);
        }
