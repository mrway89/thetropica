...
class AdminContactsController extends \crocodicstudio\crudbooster\controllers\CBController
{
    ...
    */
    ...
    public function getIndex()
    {
        ...
        $id            = Content::where('type', 'contact_us')->first();
        $data          = [];
        $data['item']  = $id;
        if ($id->other_content) {
            $data['other'] = json_decode($id->other_content);
        }
        $this->cbView('vendor/crudbooster/contact_us', $data);
    }
    ...
    public function postSaveContact(Request $request)
    {
        ...
        $array['opening']      = $request->opening;
        $array['gmap']         = $request->gmap;

        $item                  = Content::find($request->id);

        $logDesc      = 'Edit contact us';
        $other        = array_diff($array, json_decode($item->other_content, true));
        $content      = array_diff($request->only(['title', 'content']), $item->toArray());
        $newArr       = [
            'content' => $content,
            'other'   => $other,
        ];
        insertRhapsodieLog($logDesc, 'contact_us', CRUDBooster::myId(), json_encode($newArr));

        // $item->title           = $request->title;
        // $item->content         = $request->content;
        $item->other_content   = json_encode($array);
        $item->save();

        \Cache::forget('about_contact');

        CRUDBooster::redirectBack('Berhasil menyimpan contact us', 'success');
    }
}
