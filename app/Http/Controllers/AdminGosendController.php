...
class AdminGosendController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		$this->table 			   = "gosend_tracks";
		# END CONFIGURATION DO NOT REMOVE THIS LINE

		# START COLUMNS DO NOT REMOVE THIS LINE
		$this->col = [];
		$this->col[] = ["label" => "Order Code", "name" => "order_code"];
		$this->col[] = ["label" => "Status", "name" => "data", 'callback' => function ($row) {
		...
		$this->form[] = ['label' => 'Cart Id', 'name' => 'cart_id', 'type' => 'select2', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'cart,id'];
		...
		$this->form[] = ['label' => 'Voucher Id', 'name' => 'voucher_id', 'type' => 'select2', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'voucher,id'];
		...
		$this->form[] = ['label' => 'Insurance', 'name' => 'insurance', 'type' => 'number', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'No Resi', 'name' => 'no_resi', 'type' => 'text', 'validation' => 'required|min:1|max:255', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Delivery Date', 'name' => 'delivery_date', 'type' => 'datetime', 'validation' => 'required|date_format:Y-m-d H:i:s', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Midtrans', 'name' => 'midtrans', 'type' => 'textarea', 'validation' => 'required|string|min:5|max:5000', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Multi', 'name' => 'multi', 'type' => 'text', 'validation' => 'required|min:1|max:255', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Multi Code', 'name' => 'multi_code', 'type' => 'text', 'validation' => 'required|min:1|max:255', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Notif', 'name' => 'notif', 'type' => 'textarea', 'validation' => 'required|string|min:5|max:5000', 'width' => 'col-sm-10'];
		$this->form[] = ['label' => 'Gosend', 'name' => 'gosend', 'type' => 'textarea', 'validation' => 'required|string|min:5|max:5000', 'width' => 'col-sm-10'];
		...
		//$this->form[] = ["label"=>"Insurance","name"=>"insurance","type"=>"number","required"=>TRUE,"validation"=>"required|integer|min:0"];
		//$this->form[] = ["label"=>"No Resi","name"=>"no_resi","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
		//$this->form[] = ["label"=>"Delivery Date","name"=>"delivery_date","type"=>"datetime","required"=>TRUE,"validation"=>"required|date_format:Y-m-d H:i:s"];
		//$this->form[] = ["label"=>"Midtrans","name"=>"midtrans","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
		//$this->form[] = ["label"=>"Multi","name"=>"multi","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
		//$this->form[] = ["label"=>"Multi Code","name"=>"multi_code","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
		//$this->form[] = ["label"=>"Notif","name"=>"notif","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
		//$this->form[] = ["label"=>"Gosend","name"=>"gosend","type"=>"textarea","required"=>TRUE,"validation"=>"required|string|min:5|max:5000"];
		# OLD END FORM
	...
	{
		//Your code here
		$query->where('data', 'NOT LIKE', '%"status":"Completed"%');
	}
	...
	public function getEdit($id)
	{
		...
		$gotrack 			  = GosendTrack::find($id);

		$order                = Order::with('details', 'details.product', 'details.product.cover', 'logs')->where('order_code', $gotrack->order_code)->first();

		$data                 = [];
		$data['page_title']   = 'Manage Gosend';
		$data['order']        = $order;
		$data['midtrans']     = json_decode($order->midtrans);

		//Please use cbView method instead view method from laravel
		$this->cbView('vendor/crudbooster/gosend_management', $data);
	}
}
