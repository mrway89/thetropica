...
class AdminExperienceRetreatDetailController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public function cbInit()
    {
        ...
        $parent = DB::table('experience_contents')->where('id', request()->parent_id)->first();

        $counts = DB::table('experience_contents')->where('subparent_id', $parent->id)->get()->count();
        ...
        $this->form[] = ['label'=>'Parent Id', 'name'=>'subparent_id', 'type'=>'hidden', 'validation'=>'required', 'width'=>'col-sm-9', 'value'=> request('parent_id')];
        ...
        */
        $this->script_js = "
			$(document).ready(function(){
                ...
                $('input[value=\'Save & Add More\']').remove();
			});";
    ...
    */
    public function hook_before_add(&$postdata)
    {
        ...
        $checkSlug              = ExperienceContent::where('type', 'retreat_detail')->where('slug', $slug)->get()->count();
        if ($checkSlug > 0) {
            $slug = $slug . '-' . str_random(5);
        }
        $postdata['slug']       = $slug;
        $postdata['type']       = 'retreat_detail';
    }
    ...
    */
    ...
    {
        $slug                   = str_slug($postdata['title_id']);
        $checkSlug              = ExperienceContent::where('type', 'retreat_detail')->where('id', '!=', $id)->where('slug', $slug)->get()->count();
        if ($checkSlug > 0) {
            $slug = $slug . '-' . str_random(5);
        }
        $postdata['slug']       = $slug;
