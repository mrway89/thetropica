...
class AdminServiceProviderContentController extends \crocodicstudio\crudbooster\controllers\CBController
{
    public function cbInit()
    {
        ...
        $this->form[] = ['label'=>'Banner', 'name'=>'banner', 'type'=>'upload', 'upload_encrypt' => 'true', 'validation'=>'image|max:1000', 'width'=>'col-sm-10', 'help' => 'Recommended: 825 x 320 px'];
        ...
        $this->form[] = ['label'=>'Banner Title', 'name'=>'banner_title', 'type'=>'text', 'validation'=>'required|string', 'width'=>'col-sm-10'];

        $this->form[] = ['label'=>'Banner Content', 'name'=>'banner_content', 'type'=>'text', 'validation'=>'required|string', 'width'=>'col-sm-10'];
        ...
        */
        ...
        $this->script_js = '
			$(function() {
                ...
                var img = myPath + "/" + jsonArr["banner"];
                $("#form-group-banner").before("<div class=\'col-sm-10 col-sm-offset-2 mb-20\'><img src=\'" + img + "\' height=125></div>");
                $("#banner_title").val(jsonArr["banner_title"]);
                $("#banner_content").val(jsonArr["banner_content"]);
			});
		';
    ...
    */
    public function hook_before_edit(&$postdata, $id)
    {
        ...
        $banner                     = [];
        $img                        = Content::find($id);
        $imgurl                     = json_decode($img->other_content);

        if ($postdata['banner']) {
            if ($img) {
                if (Storage::exists($imgurl->banner)) {
                    Storage::delete($imgurl->banner);
                }
            }
            $banner['banner']   = $postdata['banner'];
        } else {
            $banner['banner'] = $imgurl->banner;
        }

        $banner['banner_title']     = $postdata['banner_title'];
        $banner['banner_content']   = $postdata['banner_content'];
        $postdata['other_content']  = json_encode($banner);
        unset($postdata['banner'], $postdata['banner_title'], $postdata['banner_content']);

        $logDesc      = 'Edit About Me';
        insertRhapsodieLog($logDesc, 'about_me', CRUDBooster::myId(), json_encode($postdata));
    }
    ...
    */
    ...
    public function getIndex()
    {
        ...
        $id = Content::where('type', 'service_provider')->first();
        return redirect()->route('AdminServiceProviderContentControllerGetEdit', ['id'=>$id]);
    }
}
